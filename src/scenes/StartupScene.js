...
const material = new THREE.MeshBasicMaterial({ color: 0xa0ffff });

export default class StartupScene extends Scene {
    constructor(camera){
        super(camera);
        this.m_MainCamera = camera;
        this.m_MainCamera.position.z = 5;
    ...
    OnUpdate(deltaTime){
        ...
        if (EVENTS.eventHandler.IsKeyPressed(EVENTS.KEY.ZERO)){
            this.SwitchScenes(new TestScene(this.m_MainCamera));
        }
    }
