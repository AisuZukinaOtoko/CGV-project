...
import lightningSound from "/Sounds/lightning2.wav";
...
export class LightningEffect {
  constructor(scene, camera, renderer) {
    ...
    this.loadSound(lightningSound); // Load your sound file here
  ...
  initLightning() {
    ...
    const outlinePass = new OutlinePass(
      ...
      this.scene,
      this.camera,
      this.lightningMeshes.map((l) => l.mesh)
    );
  ...
  setPositions(positions) {
    if (positions.length !== this.lightningMeshes.length) {
      console.warn(
        "Positions array length does not match number of lightning meshes."
      );
      return;
    }
    ...
    positions.forEach((position, index) => {
      ...
      lightning.strike.rayParameters.destOffset.copy(
  ...
  setPosition(position) {
    this.lightningMeshes.forEach(({ strike }) => {
      ...
      strike.rayParameters.destOffset.copy(
        position.clone().add(new THREE.Vector3(0, -5, 0))
      ); // Adjust end offset as needed
    });
  }
