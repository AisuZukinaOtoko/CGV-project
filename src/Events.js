...
class EventManager {
    ...
    IsKeyPressed(key) {
        ...
        if (key.length == 1){ // alphanumeric key
            result = this.m_AlphaNumericKeys[index].m_Value && !this.m_AlphaNumericKeys[index].m_Checked;
            if (result){
        ...
        // a special key
        result = this.m_SpecialKeys[index].m_Value && !this.m_SpecialKeys[index].m_Checked;
        if (result)
            this.m_SpecialKeys[index].m_Checked = true;
        return result;
    }

    IsKeyHeld(key) { // Does not work with any key. Only a specific key.
        var index = this.#GetKeyIndex(key);
    ...
    IsMouseButtonPressed(button){
        ...
        var result = this.m_MouseButtons[button].m_Value && !this.m_MouseButtons[button].m_Checked;
        if (result)
            this.m_MouseButtons[button].m_Checked = true;
        return result;
    }
    ...
    ClearInputs(){ // Reset all key and mouse inputs in array
...
// private methods. Why are you going further?
    #OnKeyDown = (event) => {
        ...
        if (event.key.length == 1){ // alphanumeric key
